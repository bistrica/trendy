...
def createConv3(attributes, labels, data, results):
    conv_nonlinearity = lasagne.nonlinearities.rectify
    if True:
        if True:
            X_train = attributes
            print 'X_train ', len(X_train)
            print 'len X_train ', len(X_train[0]), ' ',len(data)
            #f=9/0
            for x in X_train:
                print 'size ', x[0].size, x[0].shape
            #c=9/0
...
#        counterr+=1
#    ind+=1
#counterr=counterr*8/9

print 'SIZE Before ',len(image_list)
for i in reversed(range(len(image_list))):
    arr=image_list[i][0]
    allitems=True
    #print '=='
    for a in arr:
    ...
    if allitems:
        ...
        image_list.pop(i)
        imgs.pop(i)
print 'SIZE After ',len(image_list)
...
for im in image_list:
    print '>',len(im[0])
output_list=out
Y = [output_list[:int(per*len(output_list))],output_list[int(per*len(output_list)):int(per2*len(image_list))]]#int(len(image_list) * .25) : int(len(image_list) * .75)]
