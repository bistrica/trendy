...
public class Controller2 {
	...
	protected void load() {
		...
		Image imageToShow;
		imageToShow = getFrame("cd2.jpg");
		int SIZ_COLOR1 = SIZE;
		originalFrame.setImage(imageToShow);

		imageToShow = getFrame("cd1.jpg");
		System.out.println("B");
		int SIZ_COLOR2 = SIZE;

		double s = (double) SIZ_COLOR1 / (double) SIZ_COLOR2;
		System.out.println(">>>" + SIZ_COLOR1 + ", " + SIZ_COLOR2 + " = " + s);
	...
	private Mat segmentationThresholding(Mat frame) {
		...
		Imgproc.threshold(detectedEdges, detectedEdges, 160, 255, Imgproc.THRESH_BINARY);// 60
	...
	private Mat segmentationWatershed(Mat frame) {
		...
		Imgproc.threshold(inv3, inv3, 3, 255, Imgproc.THRESH_BINARY);
		...
		System.out.println(markers.size() + " - " + fin.size());
	...
	private Mat segmentationKMeans(Mat frame) {
		...
		Mat inv = new Mat(), clustered = new Mat();
		// detectedEdges.copyTo(inv);

		// Imgproc.threshold(inv, inv, 200, 255, Imgproc.THRESH_BINARY_INV |
		// Imgproc.THRESH_OTSU);// 160
		// Imgproc.morphologyEx(inv, inv, Imgproc.MORPH_OPEN, matEllipse);
